...
export const handler: Handler = async (event, context) => {
    ...
    try {
        ...
        // Sanitize inputs to prevent Log Injection (CWE-117) and DoS
        // Replace newlines with underscores and limit length
        const safeCommand = (command || '').toString().replace(/[\r\n]/g, '_').substring(0, 100);
        const safeUserId = (userId || '').toString().replace(/[\r\n]/g, '_').substring(0, 50);
